...
class Order extends Model
{
    ...
    const STATUS = array(0 => '未支付', 1 => '未發貨', 2 => '已發貨', 3 => '已凍結', 4 => '已退款');
    ...
    function getSendMessage()
    {
        if (count($this->cards)) {
            if (count($this->cards) == $this->count) {
                $sp228ba6 = '訂單#' . $this->order_no . '&nbsp;已支付，您購買的內容如下：';
            } else {
                if ($this->cards[0]->type === \App\Card::TYPE_REPEAT || @$this->product->delivery === \App\Product::DELIVERY_MANUAL) {
                    $sp228ba6 = '訂單#' . $this->order_no . '&nbsp;已支付，您購買的內容如下：';
                } else {
                    $sp228ba6 = '訂單#' . $this->order_no . '&nbsp;已支付，目前庫存不足，您還有' . ($this->count - count($this->cards)) . '件未發貨，請聯系商家客服發貨，';
                    $sp228ba6 .= '商家客服QQ：<a href="http://wpa.qq.com/msgrd?v=3&uin=' . $this->user->qq . '&site=qq&menu=yes" target="_blank">' . $this->user->qq . '</a><br>';
                    $sp228ba6 .= '已發貨商品見下方：';
                }
            }
        } else {
            $sp228ba6 = '訂單#' . $this->order_no . '&nbsp;已支付，目前庫存不足，您購買的' . ($this->count - count($this->cards)) . '件未發貨，請聯系商家客服發貨<br>';
            $sp228ba6 .= '商家客服QQ：<a href="http://wpa.qq.com/msgrd?v=3&uin=' . $this->user->qq . '&site=qq&menu=yes" target="_blank">' . $this->user->qq . '</a>';
        }
        return $sp228ba6;
    }

    function sendEmail($sp7c700e = false)
    {
        if ($sp7c700e === false) {
            $sp7c700e = @json_decode($this->contact_ext)['_mail'];
        }
        if (!$sp7c700e || !@filter_var($sp7c700e, FILTER_VALIDATE_EMAIL)) {
            return;
        }
        $spa07a9b = $this->getCardsArray();
        try {
            Mail::to($sp7c700e)->Queue(new OrderShipped($this, $this->getSendMessage(), join('<br>', $spa07a9b)));
            ...
            LogWriter::error('Order.sendEmail error', array('order_no' => $this->order_no, 'email' => $sp7c700e, 'cards' => $spa07a9b, 'exception' => $sp45222f->getMessage()));
            LogHelper::setLogFile('card');
        }
    }

    function sendSms($sp7a4d87 = false)
    {
        if ($sp7a4d87 === false) {
            $sp7a4d87 = @json_decode($this->contact_ext)['_mobile'];
        }
        if (!$sp7a4d87 || strlen($sp7a4d87) !== 11) {
            return;
        }
        OrderSms::dispatch($sp7a4d87, $this);
    }
}
