...
class Pay extends Controller
{
    ...
    function buy(Request $sp13451b)
    {
        ...
        if (strlen($sp9a876f) !== 32) {
            return self::renderResultPage($sp13451b, array('msg' => '提交超時，請刷新購買頁面並重新提交<br><br>
當前網址: ' . $sp13451b->getQueryString() . '
提交內容: ' . var_export($sp9a876f) . ', 提交長度:' . strlen($sp9a876f) . '<br>
若您刷新後仍然出現此問題. 請加網站客服反饋'));
        }
        if (System::_getInt('vcode_shop_buy') === 1) {
            try {
                $this->validateCaptcha($sp13451b);
        ...
        if ($sp863814->price_whole) {
            ...
            for ($sp51a993 = count($sp5248c4) - 1; $sp51a993 >= 0; $sp51a993--) {
                if ($sp1ca412 >= (int)$sp5248c4[$sp51a993][0]) {
                    $sp261316 = (int)$sp5248c4[$sp51a993][1];
                    break;
                }
            }
        }
            ...
            foreach ($spd4c074 as $sp55c7ac) {
                if ($sp55c7ac->category_id === -1 || $sp55c7ac->category_id === $sp3955fa && ($sp55c7ac->product_id === -1 || $sp55c7ac->product_id === $spcaeba2)) {
                    if ($sp55c7ac->discount_type === \App\Coupon::DISCOUNT_TYPE_AMOUNT && $sp50cd10 >= $sp55c7ac->discount_val) {
                        $sp3c9138 = $sp55c7ac;
                        $sp5698b2 = $sp55c7ac->discount_val;
                        break;
                    }
                    if ($sp55c7ac->discount_type === \App\Coupon::DISCOUNT_TYPE_PERCENT) {
                        $sp3c9138 = $sp55c7ac;
                        $sp5698b2 = (int)round($sp50cd10 * $sp55c7ac->discount_val / 100);
                        break;
                    }
                }
            }
        ...
        $sp1ae43e = $sp50cd10 - $spd81ea0;
        $sp188cde = $spd55aa5 ? System::_getInt('sms_price', 10) : 0;
        $sp50cd10 += $sp188cde;
        ...
        try {
            DB::transaction(function () use ($sp863814, $sp6b06ff, $sp3c9138, $sp6c47af, $sp05a58e, $sp9a876f, $sp1ca412, $sp15e80d, $sp6fa047, $sp188cde, $sp5698b2, $sp50cd10, $sp2e6adb, $spd81ea0, $sp1ae43e) {
                if ($sp3c9138) {
                    ...
                    $sp3c9138->count_used++;
                    $sp3c9138->save();
                    $speb1f6b = '使用優惠券: ' . $sp3c9138->coupon;
                } else {
                    $speb1f6b = null;
                }
            ...
            Log::error('Shop.Pay.buy 下單失敗', array('exception' => $sp45222f));
            return self::renderResultPage($sp13451b, array('msg' => trans('shop.pay.internal_error')));
        }
        if ($sp50cd10 === 0) {
            $this->shipOrder($sp13451b, $sp6b06ff, $sp50cd10, null);
    ...
    function pay(Request $sp13451b, $sp6b06ff)
    {
...
');
        ...
        $sp656002 = $sp6b06ff;
        return $this->goPay($sp13451b, $sp6b06ff, $sp656002, $sp2e6adb, $sp322370->paid);
    }
...
');
    ...
    {
        $spf6b1a9 = 'payNotify pay_id: ' . $sp56237f;
        Log::debug($spf6b1a9);
        $sp2e6adb = \App\Pay::where('id', $sp56237f)->first();
        if (!$sp2e6adb) {
            Log::error($spf6b1a9 . ' cannot find PayModel');
            echo 'fail';
            die;
        }
        $spf6b1a9 .= ',' . $sp2e6adb->driver;
        ...
        Log::debug($spf6b1a9 . ' notify finished: ' . (int)$sp00ac62 . '
...
');
    ...
    function shipOrder($sp13451b, $sp6b06ff, $sp07bb5f, $spbe9054)
    {
        $sp322370 = \App\Order::whereOrderNo($sp6b06ff)->first();
        if ($sp322370 === null) {
            Log::error('shipOrder: No query results for model [App\\Order:' . $sp6b06ff . ',trade_no:' . $spbe9054 . ',amount:' . $sp07bb5f . ']. die(\'success\');');
            die('success');
        }
        if ($sp322370->paid > $sp07bb5f) {
            Log::alert('shipOrder, price may error, order_no:' . $sp6b06ff . ', paid:' . $sp322370->paid . ', $amount get:' . $sp07bb5f);
            $sp322370->remark = '支付金額(' . sprintf('%0.2f', $sp07bb5f / 100) . ') 小於 訂單金額(' . sprintf('%0.2f', $sp322370->paid / 100) . ')';
            $sp322370->save();
        ...
        if ($sp322370->status === \App\Order::STATUS_UNPAY) {
            ...
            if (FundHelper::orderSuccess($sp322370->id, function ($spde9919) use ($spbe9054, &$sp322370, &$sp863814) {
                ...
                $sp863814 = $sp322370->product()->lockForUpdate()->firstOrFail();
                $sp322370->pay_trade_no = $spbe9054;
                $sp322370->paid_at = Carbon::now();
                if ($sp863814->delivery === \App\Product::DELIVERY_MANUAL) {
                    $sp322370->status = \App\Order::STATUS_PAID;
                    $sp322370->send_status = \App\Order::SEND_STATUS_CARD_UN;
                    $sp322370->saveOrFail();
                    return true;
                }
                ...
                if ($spf0473c === false) {
                    Log::alert('Shop.Pay.shipOrder: 訂單:' . $sp322370->order_no . ', 購買數量:' . $sp322370->count . ', 卡數量:' . count($spa07a9b) . ' 卡密不足(已支付 未發貨)');
                    $sp322370->status = \App\Order::STATUS_PAID;
                    $sp322370->saveOrFail();
                    return true;
                } else {
                    ...
                    $sp322370->status = \App\Order::STATUS_SUCCESS;
                    $sp322370->saveOrFail();
                    $sp863814->count_sold += $sp322370->count;
                    $sp863814->saveOrFail();
                ...
                if ($sp863814->count_warn > 0 && $sp863814->count < $sp863814->count_warn) {
                    try {
                        Mail::to($sp322370->user->email)->Queue(new ProductCountWarn($sp863814, $sp863814->count));
                    } catch (\Throwable $sp45222f) {
